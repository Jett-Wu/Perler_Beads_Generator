...
一个强大的拼豆图纸编辑器，可将图片转换为可打印的拼豆图纸，并提供 MARD 色卡匹配、图层编辑、参考图临摹、3D 预览、用量统计和高清导出。
...
- **专业编辑工具**：画笔、橡皮、填充、消除、换色、吸管、移动、复制、粘贴、镜像和基础形状。
- **多图层工作流**：支持新建、隐藏、锁定、重命名、复制、拖拽排序、只看当前图层和按图层统计用量。
- **参考图临摹**：可上传参考图，调整透明度、位置和缩放，用于手工临摹或核对图案。
- **当前图层调整**：支持亮度、对比度、饱和度、色温、色相、反色、灰阶、黑白、颜色整理和色数上限。
- **实时 3D 预览**：模拟拼豆完成后的效果，支持放大查看，并跟随圆形 / 方形豆显示。
- **用量统计**：按颜色统计颗数，支持设置每包数量，估算每种颜色所需包数。
- **高清导出**：支持导出 PNG / PDF 图纸、Excel 用量清单和 JSON 编辑记录。
- **浏览器本地处理**：图片转换和编辑在本地完成，不需要上传图片到服务器。
...
| 原图 | 工作台 | 导出图纸 |
| --- | --- | --- |
| ![Realistic source](docs/realistic-source.jpg) | ![Realistic workspace](docs/realistic-workspace.png) | ![Realistic exported pattern](docs/realistic-pattern.png) |

### 卡通风格

| 原图 | 工作台 | 导出图纸 |
| --- | --- | --- |
| ![Cartoon source](docs/cartoon-source.png) | ![Cartoon workspace](docs/cartoon-workspace.png) | ![Cartoon exported pattern](docs/cartoon-pattern.png) |
...
src/
  ...
  WorkspaceCanvas.tsx     # 2D 画布、绘制工具、参考图、移动复制等编辑逻辑
...
A powerful Perler bead pattern editor for turning images into printable bead charts with MARD color matching, layers, reference tracing, 3D preview, usage counting, and high-resolution exports.
...
- **Editing tools** including pencil, eraser, fill, clear, recolor, eyedropper, move, copy, paste, mirror, and shapes.
